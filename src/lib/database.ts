...
export const convertTodoFromDB = (dbTodo: TodoDatabase) => ({
  ...
  completedAt: dbTodo.completed_at ? new Date(dbTodo.completed_at).getTime() : undefined,
});

export const convertTodoToDB = (todo: any, userId: string): Partial<TodoDatabase> => ({
  user_id: userId,
  text: todo.text,
  color: todo.color,
  ...
  completed_at: todo.completedAt ? new Date(todo.completedAt).toISOString() : undefined,
});
...
export const insertTodo = async (todo: any, userId: string) => {
  const { data, error } = await supabase
    .from('todos')
...
export const updateTodo = async (id: number, updates: Partial<any>, userId: string) => {
  const { data, error } = await supabase
    .from('todos')
    .update({
      ...updates,
...
export const insertInboxTodo = async (todo: any, userId: string) => {
  const { data, error } = await supabase
    .from('inbox_todos')
    .insert(convertTodoToDB(todo, userId))
    .select()
...
export const updateInboxTodo = async (id: number, updates: Partial<any>, userId: string) => {
  const { data, error } = await supabase
    .from('inbox_todos')
    .update({
      ...updates,
...
export const insertCompletedTodo = async (todo: any, userId: string) => {
  const { data, error } = await supabase
    .from('completed_todos')
    .insert({
      ...convertTodoToDB(todo, userId),
      completed_at: new Date(todo.completedAt).toISOString(),
    })
    .select()
    .single();
...
export const upsertUserSettings = async (userId: string, settings: Partial<UserSettings>) => {
  const { data, error } = await supabase
    .from('user_settings')
    .upsert({
      user_id: userId,
...
export const moveTodoToCompleted = async (todo: any, userId: string, fromTable: 'todos' | 'inbox_todos') => {
  const completedTodo = {
    ...todo,
...
export const moveTodoToInbox = async (todo: any, userId: string) => {
  await insertInboxTodo(todo, userId);
  await deleteTodo(todo.id, userId);
};

export const moveTodoToMain = async (todo: any, userId: string) => {
